...
export const VulcanCard: React.FC<VulcanCardProps> = ({
  ...
  return (
    <div className={`${isDark ? 'card-dark' : 'card'} card-hover transition-all duration-300 max-w-md mx-auto sm:max-w-lg lg:max-w-xl ${className}`}>
      ...
      <div className="stack-to-row overflow-hidden">
        ...
        <div className="half-on-tablet responsive-padding">
          ...
          <h3 className={`text-center-left-on-tablet text-lg md:text-xl font-bold mb-2 ${isDark ? 'text-[var(--color-vulcan-50)]' : 'text-[var(--color-vulcan-900)]'}`}>
            {title}
          </h3>
          
          <p className={`mb-4 ${isDark ? 'text-[var(--color-vulcan-100)]' : 'text-[var(--color-vulcan-600)]'}`}>
            {description}
          </p>
          ...
          <div className="stack-to-row justify-between items-center space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            <button className={`show-only-on-mobile p-2 rounded-full ${
