...
export const ResponsiveCard: React.FC<ResponsiveCardProps> = ({
  ...
  return (
    <div className={`card card-hover shadow-lg transition-all duration-300 max-w-md mx-auto sm:max-w-lg lg:max-w-xl ${className || ''}`}>
      ...
      <div className="stack-to-row overflow-hidden">
        ...
        <div className="half-on-tablet responsive-padding">
          ...
          <h3 className="text-center-left-on-tablet text-lg md:text-xl font-bold mb-2 text-[var(--color-vulcan-800)]">
            {title}
          </h3>
          
          <p className="text-[var(--color-vulcan-600)] mb-4">{description}</p>
          ...
          <div className="flex flex-nowrap sm:flex-wrap overflow-x-auto sm:overflow-visible -mx-1 mb-4">
            {tags.map((tag, index) => (
              <span 
                key={index}
                className="inline-block bg-[var(--color-vulcan-100)] text-[var(--color-vulcan-700)] 
          ...
          <div className="stack-to-row justify-between items-center space-y-2 sm:space-y-0 sm:space-x-2">
            ...
            <button className="btn-outline hide-on-mobile">
              Save for Later
            </button>
            
            {/* Icon button shown only on mobile */}
            <button className="show-only-on-mobile p-2 rounded-full bg-[var(--color-vulcan-100)]">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--color-vulcan-600)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
